...
// void delete (SinglyCircularLL *list)
{
    if (isEmpty(list))
    {
...
element *moveCrnt(SinglyCircularLL *list, int skipNum)
{
    ...
    // printf("moveCrnt함수에서의 지워진 사람의 노드는 %d %s 입니다.\n", returnNode->num, returnNode->name);
    return returnNode;
}
...
int main()
{
    ...
    */
    ...
    while (pplList->length > 3)
    {
        ...
        // printf("main 함수에서 지워진 사람의 노드는 %d %s 입니다.\n", test->num, test->name);
        addLast(loser, test);
        // printf("loser->length = %d\n", loser->length);
        //  예상: 1 -> 2 -> ... 11
        printf("\n탈락자 명단 \n");
